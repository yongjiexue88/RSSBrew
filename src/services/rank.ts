...
function computeBuyerIntentScore(text: string): { score: number; matchReasons: string[] } {
  ...
  const strongSignals: [RegExp, number][] = [
    ...
    [/\blooking for\s+(?:[a-zA-Z-]+\s+){0,5}(?:something|software|app|system|solution|program|service)\b/i, 8],
    [/\bmoving from spreadsheets?\b/i, 8],
    [/\bhow do you automate\b/i, 6],
...
function computeWorkflowContextScore(text: string): { score: number; matchReasons: string[] } {
  ...
  const REAL_CONTEXT_PATTERNS = [
    ...
    /\bmy business\b/i,
    /\bour business\b/i,
    /\bmy (?:shop|store|crm|esp|list|lists|workflows?|setup|setups)\b/i,
    /\bour (?:shop|store|crm|esp|list|lists|workflows?|setup|setups)\b/i,
    /\bclients?\b/i,
    /\bwe currently\b/i,
    /\bwe use\b/i,
    /\bevery week\b/i,
